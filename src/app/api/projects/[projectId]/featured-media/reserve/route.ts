...
import {
  ...
  reserveInvitationImageUploadForCurrentUser,
} from '@/modules/media/invitation-image.service';
import { invitationImageReservationSchema } from '@/modules/media/invitation-image.validation';
...
export async function POST(request: Request, { params }: RouteProps) {
  ...
  const parsed = invitationImageReservationSchema.safeParse(payload);
  if (!parsed.success) {
    return privateJson({ message: parsed.error.issues[0]?.message ?? 'Data foto tidak valid.' }, { status: 400 });
  }

  try {
    const reservation = await reserveInvitationImageUploadForCurrentUser({
      projectId,
      role: parsed.data.role,
      upload: { mimeType: parsed.data.mimeType, sizeBytes: parsed.data.sizeBytes },
    });
    ...
    if (error instanceof ProjectAccessDeniedError || error instanceof InvitationImageUnavailableError) {
      return privateJson({ message: 'Foto tidak dapat ditambahkan ke undangan ini.' }, { status: 404 });
    }
    if (error instanceof GalleryMediaValidationError) {
      return privateJson({ message: error.message }, { status: 400 });
    }

    console.error('Seraya featured media reserve failed.', {
      errorName: error instanceof Error ? error.name : 'UnknownError',
      projectId,
    });
    return privateJson({ message: 'Foto belum bisa disiapkan. Coba lagi beberapa saat lagi.' }, { status: 500 });
  }
}
