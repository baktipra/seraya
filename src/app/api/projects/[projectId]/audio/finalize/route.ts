...
import {
  ...
  finalizeInvitationAudioUploadForCurrentUser,
} from '@/modules/media/invitation-audio.service';
import {
  invitationAudioAssetActionSchema,
...
export async function POST(request: Request, { params }: AudioRouteProps) {
  ...
  const parsed = invitationAudioAssetActionSchema.safeParse(payload);
  if (!parsed.success) {
    return privateJson(
  ...
  try {
    ...
    console.error('Seraya invitation audio finalize failed.', {
